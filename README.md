...
Before MechaCar launches its brand to the public, it needs to ensure that it will be fierce competitor in that market. Since the brand wants to be know as the most affordible sportscar brand, MechaCar needs to outperform the competition by having the most affordible price per horsepower across its product offering. That will be their metric of success.

First we must establish our null hypothesis. With 95% certainty, MechaCar will have a more efficient price per horsepower.

We will need to acquire datasets of the competition's lines of vehicles, including names, prices, and horsepower. This was we can establish each competitors' price per horsepower, then perform T-tests against them to see if the resulting p-value indicates that MechaCar outperforms each competitor by a statistically significant margin.
